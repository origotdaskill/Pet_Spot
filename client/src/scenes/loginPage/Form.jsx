...
const registerSchema = yup.object().shape({
  ...
  petAge: yup.string().required("required"),
});
...
const Form = () => {
  ...
  const register = async (values, onSubmitProps) => {
    ...
    const savedUserResponse = await fetch(
      "https://petspot-backend-walids-projects-fc600ccd.vercel.app//auth/register",
      {
        method: "POST",
        body: formData,
      }
  ...
  const login = async (values, onSubmitProps) => {
    const loggedInResponse = await fetch("https://petspot-backend-walids-projects-fc600ccd.vercel.app//auth/login", {
      method: "POST",
...
label="Pet Type"
  onBlur={handleBlur}
          ...
          MenuProps={MenuProps}
          error={
            Boolean(touched.petType) && Boolean(errors.petType)
          }
          helperText={touched.petType && errors.petType}
          sx={{ gridColumn: "span 2" }}
